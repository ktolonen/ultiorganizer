	...
	if (is_uploaded_file($_FILES['file']['tmp_name'])) {
		...
		if (($handle = fopen($_FILES['file']['tmp_name'], "r")) !== FALSE) {
			while (($data = fgetcsv($handle, 0, $separator)) !== FALSE) {
				...
				$series = $utf8 ? trim($data[1]) : convertToUtf8(trim($data[1]));
				$country = $utf8 ? trim($data[2]) : convertToUtf8(trim($data[2]));
				$first = $utf8 ? trim($data[3]) : convertToUtf8(trim($data[3]));
				$last = $utf8 ? trim($data[4]) : convertToUtf8(trim($data[4]));
				$jersey = $utf8 ? trim($data[5]) : convertToUtf8(trim($data[5]));
				$birthdate = $utf8 ? trim($data[6]) : convertToUtf8(trim($data[6]));
				$gender = $utf8 ? trim($data[7]) : convertToUtf8(trim($data[7]));
				$role = $utf8 ? trim($data[8]) : convertToUtf8(trim($data[8]));
