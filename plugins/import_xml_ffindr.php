...
class XMLHandler
{
    public $eventId; //event id under processing
    public $divisionId; //division id under processing
    public $teamId; //team id under processing
    public $playerId; //player id under processing
    public $profileId; //profile id under processing
    public $clubId; //club id under processing
    public $tag; //tag under processing
     ...
     */
    public function XMLToEvent($filename, $eventId = "", $mode = "new")
    {
        $this->mode = $mode;
        ...
        if ((empty($this->eventId) && isSuperAdmin()) || isSeasonAdmin($eventId)) {
            ...
            xml_set_element_handler($xmlparser, [$this, "start_tag"], [$this, "end_tag"]);
            xml_set_default_handler($xmlparser, [$this, "content"]);
     ...
     */
    public function content($parser, $content)
    {
    ...
    public function InsertToDatabase($name, $row)
    {
        ...
        switch ($name) {
            ...
            case "DIVISION":
                ...
                $cond = "name='" . DBEscapeString($row['NAME']) . "' AND season='" . $this->eventId . "'";
            ...
            case "TEAM":
                $cond = "name='" . DBEscapeString($row['NAME']) . "' AND series='" . $this->divisionId . "'";
            ...
            case "PLAYER":
                ...
                if (isset($row['FIRSTNAME'])) {
                    ...
                    $cond = "firstname='" . DBEscapeString($row['FIRSTNAME']) . "' AND lastname='" . DBEscapeString($row['LASTNAME']) . "' AND team='" . $this->teamId . "'";
                } else {
                    $row['LASTNAME'] = stripslashes($row['LASTNAME']);
                    $cond = "lastname='" . DBEscapeString($row['LASTNAME']) . "' AND team='" . $this->teamId . "'";
                }
                ...
                if ($profileId > 0) {
                    ...
                    $query = sprintf(
                        ...
                        DBEscapeString(strtoupper($row['LASTNAME'])),
                    );
     ...
     * @param array $row Data to insert: key=>field, value=>data
     */
    public function SetRow($name, $row, $cond)
    {
