...
if ($token !== '') {
	...
	$teamInfo = $teamId > 0 ? TeamInfo($teamId) : array();
	$pageTitle = !empty($teamInfo['seasonname']) ? $teamInfo['seasonname'] . " - " . _("Spiritkeeper") : _("Spiritkeeper");
	$view = iget('view');
	if (!in_array($view, array('teamgames', 'submitsotg'), true)) {
		$view = 'teamgames';
	}
	...
	if (!isLoggedIn() && $view !== 'login') {
		...
		if (!in_array($view, array('login', 'logout', 'home', 'teamgames', 'editgame'), true)) {
			$view = isLoggedIn() ? 'home' : 'login';
		}
