...
function scoreboard($id, $seriesScoreboard){
  ...
  if($seriesScoreboard){
    ...
    $scores = Sermysqli_fetch_assoc,"total", 10);
...
function defenseboard($id, $seriesDefenseboard){
  ...
  if($seriesDefenseboard){
    ...
    $defenses = Smysqli_fetch_assoc($seriesinfo['series_id'],"deftotal", 10);
...
function printSwissdraw($seasoninfo, $poolinfo){
  ...
  $games = Timetmysqli_fetch_assocfo['pool_id'], "pool", "all", "series");
  ...
  foreach($pools as $poolId){
    ...
    if (empty($template)) {
      ...
      if(empty($pool)) {
        ...
        $games = Timetamysqli_fetch_assocool_id'], "pool", "all", "series");
        while ($game = mysql_fetch_assoc($games)) {
