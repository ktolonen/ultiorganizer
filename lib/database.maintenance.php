...
function DBAutomaticMaintenancePayload($status, $target, $meta = [])
{
    $payload = [
        'mode=automatic',
        'status=' . $status,
...
function DBParseAutomaticMaintenanceFlag($raw)
{
    ...
    if ($trimmed === '') {
        return ['mode' => 'manual', 'reason' => 'empty'];
    }
    ...
    if ($lineCount === 3) {
        $expected = ['mode', 'status', 'target'];
    } elseif ($lineCount === 4) {
        $expected = ['mode', 'status', 'target', 'started_at'];
    } elseif ($lineCount === 5) {
        $expected = ['mode', 'status', 'target', 'failed_at', 'error'];
    } else {
        return ['mode' => 'manual', 'reason' => 'line_count'];
    }

    $parsed = [];
    foreach ($expected as $index => $key) {
        if (!isset($lines[$index]) || strpos($lines[$index], '=') === false) {
            return ['mode' => 'manual', 'reason' => 'malformed'];
        }

        list($lineKey, $value) = explode('=', $lines[$index], 2);
        if ($lineKey !== $key) {
            return ['mode' => 'manual', 'reason' => 'unexpected_key'];
        }
        $parsed[$lineKey] = $value;
    }

    if ($parsed['mode'] !== 'automatic') {
        return ['mode' => 'manual', 'reason' => 'invalid_mode'];
    }

    if (!in_array($parsed['status'], ['pending', 'running', 'failed'], true)) {
        return ['mode' => 'manual', 'reason' => 'invalid_status'];
    }

    if (!preg_match('/^[0-9]+$/', $parsed['target']) || (int) $parsed['target'] <= 0 || (int) $parsed['target'] !== DB_VERSION) {
        return ['mode' => 'manual', 'reason' => 'invalid_target'];
    }

    if ($parsed['status'] === 'pending' && $lineCount !== 3) {
        return ['mode' => 'manual', 'reason' => 'invalid_pending'];
    }

    if ($parsed['status'] === 'running' && $lineCount !== 4) {
        return ['mode' => 'manual', 'reason' => 'invalid_running'];
    }

    if ($parsed['status'] === 'failed' && $lineCount !== 5) {
        return ['mode' => 'manual', 'reason' => 'invalid_failed'];
    }

    if ($parsed['status'] === 'running' && $parsed['started_at'] === '') {
        return ['mode' => 'manual', 'reason' => 'missing_started_at'];
    }

    if ($parsed['status'] === 'failed' && ($parsed['failed_at'] === '' || $parsed['error'] === '')) {
        return ['mode' => 'manual', 'reason' => 'missing_failure_meta'];
    }
...
function DBReadMaintenanceState()
{
    if (!DBEnsureMaintenanceRuntimeDir()) {
        return ['mode' => 'manual', 'status' => null, 'path' => DBMaintenanceFlagPath(), 'reason' => 'runtime_dir_unavailable'];
    }

    $path = DBMaintenanceFlagPath();
    if (!is_file($path)) {
        return ['mode' => 'none', 'status' => null, 'path' => $path];
    }

    $raw = @file_get_contents($path);
    if ($raw === false) {
        return ['mode' => 'manual', 'status' => null, 'path' => $path, 'reason' => 'unreadable'];
    }
...
function DBMarkAutomaticUpgradeFailed($message)
{
    ...
    DBWriteMaintenanceFlag(DBAutomaticMaintenancePayload('failed', DB_VERSION, [
...
function DBRunAutomaticUpgrade()
{
    ...
    try {
        if (!DBWriteMaintenanceFlag(DBAutomaticMaintenancePayload('running', DB_VERSION, [
...
function DBHandleMaintenanceState()
{
    ...
    if ($state['mode'] === 'automatic') {
        if ($dbVersion === DB_VERSION && in_array($state['status'], ['pending', 'running'], true)) {
            if (DBRemoveAutomaticMaintenanceFlag()) {
                return;
            }
    ...
    if (!DBCreateAutomaticMaintenanceFlag()) {
        ...
        DBRenderMaintenanceResponse(['mode' => 'automatic', 'status' => 'failed']);
    }
