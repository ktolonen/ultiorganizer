...
function ToInternalTimeFormat($timestamp)
{
    ...
    } elseif (!uo_strptime($timestamp, DATE_FORMAT)) {
 ...
 */
function resolveViewPath($view, $baseDir, $default = 'frontpage', $deny = [])
{
    // Default when empty
    if (!$view) {
        $view = $default;
...
{
    // Not empty(), which would also delete a comment of "0" -- a scoresheet
    // history restore replays a stored comment through here.
