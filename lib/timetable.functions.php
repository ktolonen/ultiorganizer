...
function TournamentView($games, $grouping=true){
	...
	while($game = mysql_fetch_assoc($games)){
		...
		if($game['reservationgroup'] != $prevTournament 
			...
			if($grouping){
				echo "<h1>". utf8entities(U_($game['reservationgroup'])) ."</h1>\n";
			}
			$prevPlace="";
		...
		if(JustDate($game['starttime']) != $prevDate || $game['place_id'] != $prevPlace){
			...
			echo "<h3>";
			echo DefWeekDateFormat($game['starttime']);
			echo " ";
			echo "<a href='?view=reservationinfo&amp;Reservation=".$game['reservation_id']."'>";
			echo utf8entities(U_($game['placename']));
			echo "</a>";
			echo "</h3>\n";
			$prevPool="";
		}
...
function SeriesView($games, $date=true, $time=false){
	...
	while($game = mysql_fetch_assoc($games)){
		if($game['series_id'] != $prevSeries 
			...
			echo "<h1>". utf8entities(U_($game['seriesname'])) ."</h1>\n";
		}	
		...
		if($game['pool'] != $prevPool){
			...
			echo "<table cellpadding='2' border='0' cellspacing='0'>\n";
			$isTableOpen = true;
			echo PoolHeaders($game);
		}				
	
	//function GameRow($game, $date=false, $time=true, $field=true, $series=false,$pool=false,$info=true)
	echo GameRow($game, true, true, true, false, false, true, $rss);
...
function PlaceView($games, $grouping=true){
	...
	while($game = mysql_fetch_assoc($games)){
		...
		if($game['place_id'] != $prevPlace || $game['fieldname'] != $prevField || JustDate($game['starttime']) != $prevDate){
			...
			echo "<table cellpadding='2' border='0' cellspacing='0'>\n";
			$isTableOpen = true;
			echo PlaceHeaders($game, true);
		}

		if($isTableOpen){
			//function GameRow($game, $date=false, $time=true, $field=true, $series=false,$pool=false,$info=true)
			echo GameRow($game, false, true, false, true, true, true,$rss);
			}
...
function ExtTournamentView($games){
	...
	while($game = mysql_fetch_assoc($games)){
		if($game['reservationgroup'] != $prevTournament 
			...
			echo "<tr><td><h1 class='pk_h1'>". utf8entities(U_($game['reservationgroup'])) ."</h1></td></tr>\n";
		}	
		...
		if($game['place_id'] != $prevPlace || $game['fieldname'] != $prevField ||JustDate($game['starttime']) != $prevDate){
			...
			echo utf8entities(U_($game['placename'])) ." "._("Field")." ".utf8entities($game['fieldname'])."</td></tr></table></td></tr>\n";
			echo "<tr><td><table width='100%' class='pk_table'>\n";
			$isTableOpen = true;
		}
		
		echo "<tr><td style='width:10px' class='pk_tournament_td2'>", DefHourFormat($game['time']) ,"</td>";
		if($game['hometeam'] && $game['visitorteam']){
			echo "<td style='width:100px' class='pk_tournament_td2'>". utf8entities($game['hometeamname']) ."</td>
			...
			if((intval($game['homescore'])+intval($game['visitorscore']))==0)
				echo "<td style='text-align: center;width:8px' class='pk_tournament_td2'>?</td>
				...
				echo "<td style='text-align: center;width:8px' class='pk_tournament_td2'>". intval($game['homescore']) ."</td>
					<td style='text-align: center;width:5px' class='pk_tournament_td2'>-</td>
					<td style='text-align: center;width:8px' class='pk_tournament_td2'>". intval($game['visitorscore']) ."</td>";
		}else{
			echo "<td style='width:100px' class='pk_tournament_td2'>". utf8entities($game['phometeamname']) ."</td>
			<td style='width:5px' class='pk_tournament_td2'>-</td>
		...
		echo "<td style='width:5px' class='pk_tournament_td2'></td>";
		echo "<td style='width:50px' class='pk_tournament_td2'>", utf8entities($game['seriesname']) ,"</td>";
		echo "<td style='width:100px' class='pk_tournament_td2'>", utf8entities($game['poolname']) ,"</td>";
		echo "</tr>\n";
...
function ExtGameView($games){
	...
	$prevTimezone = "";
	$isTableOpen = false;
	echo "<table style='white-space: nowrap' width='95%'>";
	...
	while($game = mysql_fetch_assoc($games)){
		...
		if($game['place_id'] != $prevPlace || $game['fieldname'] != $prevField || JustDate($game['starttime']) != $prevDate){
			...
			echo "<tr><td><table width='100%' class='pk_table'>";
			echo "<tr><th class='pk_teamgames_th' colspan='12'>";
			echo DefWeekDateFormat($game['starttime']) ." ". utf8entities(U_($game['placename']))." "._("Field")." ".utf8entities($game['fieldname']);
			echo "</th></tr>\n";
			$isTableOpen = true;
		}
		
		echo "<tr><td style='width:15%' class='pk_teamgames_td'>", DefHourFormat($game['time']) ,"</td>";
		if($game['hometeam'] && $game['visitorteam']){
			echo "<td style='width:36%' class='pk_teamgames_td'>". utf8entities($game['hometeamname']) ."</td>
			<td style='width:3%' class='pk_teamgames_td'>-</td>
			<td style='width:36%' class='pk_teamgames_td'>". utf8entities($game['visitorteamname']) ."</td>";
			if((intval($game['homescore'])+intval($game['visitorscore']))==0)
				echo "<td style='text-align: center;width:4%' class='pk_teamgames_td'>?</td>
					<td style='text-align: center;width:2%' class='pk_teamgames_td'>-</td>
					<td style='text-align: center;width:4%' class='pk_teamgames_td'>?</td>";
			else
				echo "<td style='text-align: center;width:4%' class='pk_teamgames_td'>". intval($game['homescore']) ."</td>
					<td style='text-align: center;width:2%' class='pk_teamgames_td'>-</td>
					<td style='text-align: center;width:4%' class='pk_teamgames_td'>". intval($game['visitorscore']) ."</td>";
		}else{
			echo "<td style='width:36%' class='pk_teamgames_td'>". utf8entities($game['phometeamname']) ."</td>
			<td style='width:3%' class='pk_teamgames_td'>-</td>
...
function PrintTimeZone($timezone){
	echo "<p class='timezone'>"._("Timezone").": ".utf8entities($timezone).". ";
	if(class_exists("DateTime") && !empty($timezone)){
		$dateTime = new DateTime("now", new DateTimeZone($timezone));
		echo _("Local time").": ".DefTimeFormat($dateTime->format("Y-m-d H:i:s"));
		}
	echo "</p>";
}
...
function TimetableGames($id, $gamefilter, $timefilter, $order, $groupfilter="")
	...
	switch($order)
		...
		case "time":
			$query .= " ORDER BY pp.time ASC, game_id ASC";
			break;
