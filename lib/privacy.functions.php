...
// How many uo_scoresheet_history snapshots the privacy scans hold in memory at once.
...
function PrivacyCollectPlayerReportData($playerId)
{
    ...
    $accreditationLog = [];
    $eventLog = [];
    $scoresheetHistorySnapshotRows = [];
    $scoresheetHistoryDetailRows = [];
    $imageInfo = null;
    $accreditationIds = [];
    $playerLogTargets = [];
    ...
    if ($playerIdList !== '') {
        ...
        $scoresheetHistorySnapshotRows = PrivacyPlayerScoresheetHistorySnapshotRows($playerIds);
        $scoresheetHistoryDetailRows = PrivacyPlayerScoresheetHistoryDetailRows($playerIds);
    }
    ...
    return [
        ...
        'url_rows' => $urls,
        'scoresheet_history_snapshot_rows' => $scoresheetHistorySnapshotRows,
        'scoresheet_history_detail_rows' => $scoresheetHistoryDetailRows,
    ];
}

/**
 * Extract this player's own values embedded in scoresheet-history snapshots.
 *
 ...
 */
function PrivacyPlayerScoresheetHistorySnapshotRows($playerIds)
{
    if (empty($playerIds)) {
        return [];
    }
 ...
 *
 * uo_scoresheet_history.detail keys a change to the player it is about -- the roster
 ...
 */
function PrivacyPlayerScoresheetHistoryDetailRows($playerIds)
{
    if (empty($playerIds)) {
        return [];
    }
...
function PrivacyRenderPlayerReportText($playerId, $adminUserId)
{
    ...
    PrivacyAppendRowsSection($lines, 'Linked profile row', empty($data['profile_row']) ? [] : [$data['profile_row']]);
    ...
    PrivacyAppendRowsSection($lines, 'Game history snapshot rows', $data['scoresheet_history_snapshot_rows']);
    PrivacyAppendRowsSection($lines, 'Game history change rows', $data['scoresheet_history_detail_rows']);
...
function PrivacyRenderUserReportText($userId, $adminUserId)
{
    ...
    PrivacyAppendRowsSection($lines, 'Game history rows', $data['scoresheet_history_rows']);
...
function PrivacyAnonymizePlayer($playerId, $adminUserId)
{
    ...
    try {
        ...
        // uo_scoresheet_history.snapshot embeds player names as free text, keyed by
...
function PrivacyDeleteUserData($userId, $adminUserId)
{
    ...
    try {
        ...
        DBQuery(sprintf(
            "UPDATE uo_scoresheet_history SET user_id='-', ip=NULL WHERE user_id='%s'",
            DBEscapeString($userId),
        ));
