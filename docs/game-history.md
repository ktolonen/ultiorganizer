...
Every mutator above has already checked its own permission before calling into `GameHistoryRecord()`/`GameHistorySnapshotIfNeeded()`, but neither of those two write helpers enforced anything itself, so a future caller could have forged an audit row or captured a snapshot for an arbitrary game id. Both now call `GameHistoryAuthorized($gameId)` after the disabled-setting/suppression/`$gameId` checks (which run first and unchanged, including under `$force`). It accepts the union of every right actually used by a caller in the table above: `hasEditGameEventsRight($gameId)`, `hasEditGamePlayersRight($gameId)`, `hasAddMediaRight()`, or `hasAccredidationRight()` against either of the game's two teams -- narrower than that would silently stop recording for an accreditation-only or media-only admin.
