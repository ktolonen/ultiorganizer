...
Excluded from this: `GameAddScore()`, `GameAddScoreEntry()` and `GameRemoveScore()`, the per-goal paths, which never call it -- see the comment on `GameRemoveScore()`. Those are still covered indirectly: `GameRemoveAllScores()`, called by the desktop save's bulk rewrite, snapshots before clearing the point sequence, so a save built from several per-goal calls still has exactly one restore point. `GameUpdateResult()` (the "still ongoing" save) also does not snapshot, because it does not clear anything -- there is nothing to capture before it runs.

`GameHistoryRestore()` (`lib/gamehistory.functions.php`) force-captures the state it is about to replace as its own snapshot, even while history recording is otherwise suppressed for the duration of its own replay, so a restore can itself be undone.
...
- **An acknowledged player who has since changed teams degrades to a warning, not a `die()`.** The up-front guard above checks `hasAccredidationRight()` against the team the snapshot recorded for that player. `AcknowledgeUnaccredited()` re-checks the player's *current* team and `die()`s on a mismatch, which would abort the replay partway through if a player moved teams between the snapshot and the restore. `GameHistoryRestorePlayers()` re-checks the current team itself before calling it, so this case is reported as a warning and the replay continues instead of crashing mid-restore.
