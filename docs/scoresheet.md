...
- a change of fixture identity bumps it too, although the desktop editor does not rewrite those columns. The editor writes its goals *against* the fixture -- `ishomegoal` and the two rosters are read from it -- so a sheet opened before the change would otherwise attach its points to the replacement teams. Six functions change a fixture, in two shapes:
  - `SetGame()` when it changes either team, and `GameChangeHome()` on a swap, both writing one game by id and calling `GameRevisionBump()`;
  - `TeamMove()`, `PoolMakeMoves()`, `PoolMakeMove()` and `PoolUndoMove()`, which fill a playoff slot with the team that advanced into it (or clear it again on an undo). These are bulk writes keyed on the scheduling-name slots and can touch several games at once, so they carry `revision=revision+1` in the same `UPDATE` rather than calling the helper -- the set-based form of it, atomic with the fixture write and matched to exactly the rows it touched.

  Each of those bulk statements also requires the assignment to actually differ, through the null-safe `NOT (col <=> val)` (or `col IS NOT NULL` for the two that clear). `PoolMovingsToPool()` returns its rows regardless of `ismoved`, so `PoolConfirmMoves()` is repeatable by design, and an ungated bump would invalidate every open scoresheet on each repeat. The null-safe comparison matters: a plain `!=` is `NULL` for an unset column and would skip exactly the pseudo-team rows a move exists to fill.

  What does **not** bump is a game changing pools without changing teams -- `SetGame()`'s `$params['pool']`, which moves a game to another pool in the timetable. The same two teams stay on the same sides, so a sheet opened beforehand is still about the right fixture. Do not read that as covering playoff advancement, which replaces the teams and is the case above.
- rows the desktop editor leaves alone deliberately do not bump it. Spirit scores, player lists, the defense sheet and most of the clock would otherwise make the scorekeeper collide with itself on every point,
- the clock is split rather than excluded whole. `GameTimeStart()` and `GameTimeReset()` write `isongoing` and `hasstarted`, which the desktop save rewrites, so both bump -- otherwise a sheet opened beforehand could finalize a game Scorekeeper had just started, or restart one it had just reset. `GameTimePause()`, `GameTimeResume()` and `GameTimeSetElapsed()` touch only the timer columns and do not bump. Both bumps are gated on the write having changed a row, since resetting an already reset clock, or starting one twice inside the same second, changes nothing and must not cost an open sheet its save; the `timer` history rows are gated with them, and `GameTimeSetElapsed()` gates its own for the same reason -- the Scorekeeper form pre-populates the adjustment with the current elapsed time,
...
`ScoresheetHistoryRestore()` also rewrites the sheet, and bumps the revision rather than checking it. It is a deliberate, snapshot-backed action restricted to event admins.
