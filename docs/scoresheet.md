...
- a change of fixture identity bumps it too, although the desktop editor does not rewrite those columns: `SetGame()` when it changes either team, and `GameChangeHome()` on a swap. The editor writes its goals *against* the fixture -- `ishomegoal` and the two rosters are read from it -- so a sheet opened before the change would otherwise attach its points to the replacement teams. A pool move does not bump: it leaves the same two teams on the same sides,
