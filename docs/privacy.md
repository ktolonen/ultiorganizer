...
- `uo_event_log`
- `uo_accreditationlog`
- `uo_scoresheet_history` for rows where `user_id` matches the selected `userid`, excluding the `snapshot` column, which is game data rather than that user's data. `UserUpdateInfo()` rewrites `user_id` in `uo_scoresheet_history` and `uo_event_log` when a login is renamed, so an account's own history follows it rather than being stranded under the old name -- and cannot be inherited by whoever is given that name next
