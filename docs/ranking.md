...
`printCrossmatchPool()` in `poolstatus.php` renders one row per game and reads the rows as those ranked pairs: the first row is pool slots 1 and 2, the second row slots 3 and 4, and so on. The winner and loser continuation pools shown for a row come from `PoolGetMoveToPool()` for those two slot numbers, so the row order has to follow the slots and not just the game list order. `TimetableGames($poolId, "pool", "all", "crossmatch")` therefore orders by the home participant's slot in the pool. Games generated by `GenerateGames()` always pair consecutive slots with the lower slot at home, so this reproduces the generated game order.

The home side may still be an unresolved placeholder (a `uo_scheduling_name` row moved in through `uo_moveteams`) while the pool it comes from is unfinished. Such a game has no `uo_team_pool` row for its home side, so the order falls back to `uo_moveteams.torank` for the placeholder's move into this pool. Without that fallback these games sort to the top of the list on a `NULL` rank, which both scrambles the row order and shifts every following row onto the wrong pair of continuation pools. Rows whose slot cannot be resolved at either level are listed last for the same reason.
